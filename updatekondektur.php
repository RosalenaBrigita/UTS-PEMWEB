...
<body>
    ...
    <section class="dashboard">
        ...
        <h1><center>Update Data Kondektur</center></h1>
        <?php
            $id_kondektur = $_GET['id_kondektur'];
            $data = mysqli_query($conn, "SELECT * FROM kondektur WHERE id_kondektur='$id_kondektur'");
            while ($print = mysqli_fetch_array($data)){
        ?>
        
        <form action="updatekondektur1.php" method="post">
            <table id="db">
                <tr>
                    <td width="200">ID Kondektur</td>
                    <td>:</td>
                    <td><input type="number" name="id_kondektur" value="<?php echo $print ['id_kondektur'];?>" readonly></td>
                </tr>
                <tr>
                    <td>Nama</td>
                    <td>:</td>
