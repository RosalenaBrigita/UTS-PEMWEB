...
<head>
    ...
    <style>
        ...
        input[type=text], input[type=number], input[type=date], input[type=text-area], select[type=text] {
            width: 100%;
            padding: 12px 20px;
            margin: 8px 0;
            border:0px;
        ...
        input[type=text]:hover, input[type=number]:hover, input[type=date]:hover, select[type=text]:hover{
            opacity:1;
            border:2px;
            border-color:#06194d;;
            border-style:groove;
...
<body>
    ...
    <section class="dashboard">
        ...
        <h1><center>Insert Data Trans UPN</center></h1>

            <form action="savetransupn.php" method="post">
            <table id="db">
                <tr>
                    <td width="200">ID Trans UPN</td>
                    <td>:</td>
                    <td><input type="number" name="id_trans_upn" required></td>
                </tr>
                <tr>
                    <td>ID Kondektur</td>
                    <td>:</td>
                    <td><select type="text" name="id_kondektur">
                        <option>---</option>
                        <?php
                        include "conn.php";
                        $query = mysqli_query($conn,"select * from kondektur;") or die (mysqli_connect_error($conn));
                        while($data = mysqli_fetch_array($query)){
                            echo "<option value=$data[id_kondektur]> $data[id_kondektur] $data[nama] </option>";
                        }
                        ?>
                        </select>
                </tr>
                <tr>
                    <td>ID Bus</td>
                    <td>:</td>
                    <td><select type="text" name="id_bus">
                        <option>---</option>
                        <?php
                        include "conn.php";
                        $query = mysqli_query($conn,"select * from bus;") or die (mysqli_connect_error($conn));
                        while($data = mysqli_fetch_array($query)){
                            echo "<option value=$data[id_bus]> $data[id_bus] $data[plat] </option>";
                        }
                        ?>
                        </select>
                </tr>
                    ...
                    <td><select type="text" name="id_driver">
                        ...
                        while($data = mysqli_fetch_array($query)){
                            echo "<option value=$data[id_driver]> $data[id_driver] $data[nama] </option>";
                        }
                        ?>
                        </select>
                </tr>
                <tr>
                    ...
                    <td>:</td>
                    <td><input type="text" name="jumlah_km" size="30" required></td>
                </tr>
                <tr>
                    <td>Tanggal</td>
                    <td>:</td>
